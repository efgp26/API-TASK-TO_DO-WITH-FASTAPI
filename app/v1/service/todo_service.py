...
def create_task(task: todo_schema.TodoCreate, user: user_schema.User):
    ...
    return todo_schema.Todo(id=db_task.id, title=db_task.title, is_done=db_task.is_done, created_at=db_task.created_at)
...
def get_tasks(user: user_schema.User, is_done: bool = None):
    ...
    for task in tasks_by_user:
        ...
        list_title.append(task.title)
        list_is_done.append(task.is_done)
        list_create_at.append(task.created_at)

    dict_tasks.update(        
            id = list_id,
            title = list_title,
            is_done = list_is_done,
            created_at = list_create_at        
    )        
    return dict_tasks
